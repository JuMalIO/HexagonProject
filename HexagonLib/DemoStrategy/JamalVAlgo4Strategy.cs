...
{
    public class JamalVAlgo4Strategy : IStrategy
    {
        private Random _rnd = new Random(DateTime.Now.Millisecond);

        private List<string> myCellIds = new List<string>();

        public JamalVAlgo4Strategy()
        {
        }
        ...
        public Transaction Turn(MyCell[] myBlocks)
        {
            ...
            if (myBlocks.Length >= myCellIds.Count)
            {
                AddId(myBlocks);
                ...
                if (target == null)
                    target = GetTargetAggressively2(myBlocks, source.Resources);
                if (target == null)
                    target = GetTargetOther(myBlocks, source.Resources);
            }
            else
            {
                target = GetTargetAggressively(myBlocks, source.Resources);
            }
        ...
        private void AddId(MyCell[] myBlocks)
        {
            ...
            {
                if (!myCellIds.Contains(myBlocks[i].Id))
                {
                    myCellIds.Add(myBlocks[i].Id);
                }
            }
        }
        ...
        {
            var myCell = GetMyCellById(myBlocks, myCellIds.LastOrDefault());
            string previousId = null;
            if (myCellIds.Count > 2)
                previousId = myCellIds[myCellIds.Count - 2];
        ...
        private NeighbourCell GetTargetAggressively2(MyCell[] myBlocks, int sourceResources)
        {
            int c = 0;
            ...
            for (int i = 0; i < myBlocks.Length; i++)
            {
                for (int ii = 0; ii < myBlocks[i].Neighbours.Length; ii++)
                {
                    ...
                    {
                        int count = myBlocks[i].Neighbours.Count(item => item.Owner == CellOwner.Own);
                        if (countNeighbourCell == null || count > c || myBlocks[i].Neighbours[ii].Resources <= countNeighbourCell.Resources && count >= c)
                        {
                            c = count;
        ...
        private NeighbourCell GetTargetAggressively(MyCell[] myBlocks, int sourceResources)
        {
            int c = 0;
            NeighbourCell countNeighbourCell = null;
            for (int i = 0; i < myBlocks.Length; i++)
